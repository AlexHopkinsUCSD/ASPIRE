...
from app.infrastructure.database.repositories.concept import ConceptRepository, ConceptToDomainRepository, ConceptToModuleRepository, ConceptToConceptRepository
...
class ContextConstructor:
    def __init__(
        ...
        self.c_to_d_repo: CToDRepoProtocol = ConceptToDomainRepository()
    ...
    async def domain_concepts(self, params=None) -> ContextCollection:
        if not self.file_contents or not self.course_id:
            message = f"METHOD: .domain_concepts() missing required argument(s): {'course_id' if not self.course_id else ''} {'content_files' if not self.file_contents else ''}"
            raise ValidationError(origin="ContextConstructor", status_code=400, message=message)

        concepts_obj = await self.concept_repo.get_many(filters=ConceptFilter(course_id=self.course_id), domain_init_mode=True)
