...
from .errors.validation_errors import StateValidationError, TokenValidationError, NonceValidationError
...
class AuthResponseValidator:
    ...
    async def validate_token(self) -> tuple[dict, str, str]:
        """
        Validates the token as per https://www.imsglobal.org/spec/security/v1p0/#authentication-response-validation

        :returns: The decoded and validated id_token in the form of a dictionary
        ...
        target_link_uri, storage_target = await self._validate_state_and_nonce(claims=unevaluated_claims, request_form=request_form)
        ...
        platform_public_key = await get_platform_public_key(settings=self._settings)
        ...
        try:
            ...
            payload = jwt.decode(token, key=key, algorithms=['RS256'], audience=["268300000000000006"])
        ...
        return payload, target_link_uri, storage_target


    async def _validate_state_and_nonce(self, claims: dict, request_form) -> tuple[str, str]:
        """
        ...
        try:
            ...
            nonce_session = self._session_cache.get(cache_id=jwt_nonce, store="nonce")
            ...
            return nonce_session.target_link_uri, nonce_session.storage_target
