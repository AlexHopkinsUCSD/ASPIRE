...
<template>
  <div class="nav-container" :class="{ collapsed: isCollapsed }">
    <div class="tabs-container">
      ...
      <div class="nav-btn logo-btn">
        <template v-if="isCollapsed">
          <img 
            src="/static/assets/aspire_new.png"
    ...
    <button @click="toggleCollapse" class="collapse-button" :class="{ 'collapsed': isCollapsed }">
      <img :src="isCollapsed ? '/static/assets/icon-SideBar.png' : '/static/assets/icon-SideBar.png'" />
    </button>
  </div>
</template>
...
.nav-container {
  ...
  height: 100vh; /* Set the height to 100vh to take up the entire viewport height */
  display: flex;
